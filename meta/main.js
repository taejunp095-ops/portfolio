...
function renderCommitInfo(data, commits) {
  ...
  dl.append('dd').text(
    lines.length ? d3.mean(lines, (d) => d.length).toFixed(2) : 0
  );

  dl.append('dt').text('Maximum depth');
  dl.append('dd').text(d3.max(lines, (d) => d.depth) ?? 0);

  dl.append('dt').text('Longest line');
  dl.append('dd').text(d3.max(lines, (d) => d.length) ?? 0);
}
...
function onTimeSliderChange() {
  commitProgress = Number(d3.select('#commit-progress').property('value'));

  commitMaxTime = timeScale.invert(commitProgress);

  d3.select('#commit-time-display').text(
...
function renderSelectionCount(selection) {
  const selectedCommits = selection
    ? commits.filter((d) => isCommitSelectedGlobal(selection, d))
    : [];
...
function isCommitSelectedGlobal(selection, commit) {
  ...
  const circle = circles.find((c) => c.__data__ === commit);
